...
try:
    ...
    # Fallback stub so the class definition below doesn't crash
...
class DropFolderHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        # Wait for file to be fully written (Windows needs more time)
        for _ in range(10):
            time.sleep(0.5)
...
class FilesystemWatcher(BaseWatcher):
    ...
    def handle_new_file(self, source: Path) -> Path:
        ...
        # Skip if file no longer exists (e.g. temp files)
        if not source.exists():
            self.logger.warning(f"File gone before processing: {source.name}")
            return source
    ...
    def _read_preview(self, source: Path, max_chars: int = 500) -> str:
        """Return a text preview for plain-text files, empty string otherwise."""
        text_types = {".txt", ".md", ".csv", ".log", ".json", ".xml", ".html", ".py"}
        if source.suffix.lower() not in text_types:
            return ""
        try:
    ...
    def _write_action_md(self, source: Path, inbox_dest: Path, timestamp: str) -> Path:
        ...
        preview = self._read_preview(source)
        preview_section = (
            f"\n## File Preview\n\n```\n{preview}\n```\n" if preview else ""
        )
